...
public class CarControllerTests extends ControllerTestCase {
	...
	public void an_admin_user_can_post_a_new_car() throws Exception {
		...
		MvcResult response = mockMvc.perform(
				post("/api/cars/post?make=Honda&model=Civic&year=2014")
						.with(csrf()))
				.andExpect(status().isOk()).andReturn();
	...
	public void admin_can_edit_an_existing_car() throws Exception {
		...
		String requestBody = mapper.writeValueAsString(carUpdated);
		...
		verify(carRepository, times(1)).save(carUpdated); // should be saved with correct user
		String responseString = response.getResponse().getContentAsString();
